...
namespace AlgoritmDM.Com
{
    ...
    public static class ProviderFarm
    {
        ...
        public static UProvider CurrentPrv
        {
            get
            {
                // Если мы в режиме без базы то создадим условную базу чтолбы потом не нужно было ничего делать
                if (Com.Config.Mode == ModeEn.NotDB && _CurrentPrv == null)
                {
                    _CurrentPrv = new UProvider("ODBCprv", "NotDB");
                }
                return _CurrentPrv; 
            }
            private set 
